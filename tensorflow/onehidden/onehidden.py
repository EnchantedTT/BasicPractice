...
with graph.as_default():
	...
	#hidden
	h_weights = tf.Variable(tf.truncated_normal([num_hidden, num_labels]))
	h_biases = tf.Variable(tf.zeros([num_labels]))
	h_logits = tf.matmul(a, h_weights) + h_biases

	loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(labels=tf_train_labels, logits=h_logits))

	# Optimizer.
	optimizer = tf.train.GradientDescentOptimizer(0.1).minimize(loss)

	# Predictions for the training, validation, and test data.
	train_prediction = tf.nn.softmax(h_logits)
